...
do
    ...
    if [ -d ${DATA_DIR}/${RES_FILE_NAME} ]; then
        ...
        cmd="estimate_hic_resolution.py ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}.allValidPairs ${GENOME_SIZE} -o ${QC_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}.allValidPairs -t ${N_CPU}"
        exec_cmd $cmd >> ${ldir}/qc.log 2>&1
